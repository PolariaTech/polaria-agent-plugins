...
| `gate-calidad-tecnica` | Ejecuta el Gate de Calidad Tecnica Pre-Merge v1.2 de Polaria antes de cada push de codigo (los workflows n8n van por `gate-calidad-n8n`): skill `gate-calidad-tecnica-pre-merge-polaria` + subagente aislado y de solo lectura `revisor-tecnico-pre-merge` (8 criterios, incluye build de despliegue) + hook que bloquea `git push` sin veredicto (en la IA y, via un `pre-push` de git que el plugin instala al abrir sesion, tambien desde la terminal o el panel de Git) | Node.js en la maquina del dev (para el hook y el build). Trae el MCP de Vercel (temporal, se reemplazara por AWS): cada dev inicia sesion en Vercel la primera vez y debe ser miembro del equipo de Vercel para que el criterio 8 lea el comando de build real; sin eso usa `vercel.json`/`package.json`. Opcional: MCP de Linear y GitHub (o `gh`) para leer el tipo de issue y publicar el reporte |
