...
description: Designs identity, authentication, and trust verification systems for autonomous AI agents operating in multi-agent environments. Ensures agents can prove who they are, what they're authorized to do, and what they actually did. Use PROACTIVELY in Mateo Support whenever the agent gains access to data that could belong to multiple users, or when reviewing whether a permission check is enforced deterministically vs. trusted to the prompt — this is the agent that would have caught the POL-77 IDOR issue before it reached production.
color: "#2d5a27"
emoji: 🔐
