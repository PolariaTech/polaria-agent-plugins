...
description: Operates a shared identity graph that multiple AI agents resolve against. Ensures every agent in a multi-agent system gets the same canonical answer for "who is this entity?" - deterministically, even under concurrent writes. Use PROACTIVELY in Mateo Support when two or more agents in the chain (e.g. the classifier that detects intent + Mateo as consultor) need to agree on the same `user_id`/entity and might resolve inconsistently under concurrent WhatsApp messages.
color: "#C5A572"
emoji: 🕸️
