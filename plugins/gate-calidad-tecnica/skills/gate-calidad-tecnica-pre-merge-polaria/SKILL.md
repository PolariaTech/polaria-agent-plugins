...
description: Ejecuta el gate de calidad técnica de Polaria sobre los commits locales de un cambio, ANTES de hacer push o abrir PR, despachando un revisor aislado. Úsala SIEMPRE que un dev diga que terminó un cambio, pida revisar un diff/rama/PR, pregunte "¿puedo subir esto?", "revisa esto antes de hacer push", "terminé el cambio, ¿está listo?", "¿puedo mergear esto?", "ya corregí el hallazgo de la auditoría", o pegue un diff pidiendo el segundo par de revisión — y SIEMPRE que un push quede bloqueado por el hook del gate — incluso si no menciona "gate de calidad" o "revisión de pares" por nombre. Aplica a todo cambio sin excepción, incluyendo issues Urgent. No la uses para la validación de "Done" (comportamiento funcional, cubierta por otro protocolo), ni para auditar un sistema completo (eso es `auditoria-tecnica-polaria`), ni para publicar el resultado en GitHub/Linear sin que el dev lo pida.
---
...
- **`APROBADO`:** dile al dev que puede hacer `push` y abrir el PR.
...
El hook se dispara cuando el commit que se va a publicar no tiene marca `APROBADO` ni `RECHAZADO_JUSTIFICADO`. No lo esquives (ni borrando el hook, ni escribiendo la marca sin revisión, ni pidiendo al dev que haga el `push` desde su terminal para saltarlo): corre esta skill desde el Paso 1 sobre el commit actual.
